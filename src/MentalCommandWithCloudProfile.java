...
public class MentalCommandWithCloudProfile implements Runnable {
	...
	public static String profileName = "Main Profile";
	...
	public static void main(String[] args) throws InterruptedException {
		...
		String userName = "bb88";
		String password = "Brinder88";
	...
	private static void live(){
		int lastMentalCommand = Integer.MAX_VALUE;
		while(true) {
		state = Edk.INSTANCE.IEE_EngineGetNextEvent(eEvent);
		if(state == EdkErrorCode.EDK_OK.ToInt()){
			int eventType = Edk.INSTANCE.IEE_EmoEngineEventGetType(eEvent);
			...
			if (eventType == Edk.IEE_Event_t.IEE_EmoStateUpdated.ToInt()){
				Edk.INSTANCE.IEE_EmoEngineEventGetEmoState(eEvent,eState);
				int currentCommand=EmoState.INSTANCE.IS_MentalCommandGetCurrentAction(eState);
				if(currentCommand!=lastMentalCommand) {
					if(currentCommand==EmoState.IEE_MentalCommandAction_t.MC_NEUTRAL.ToInt())
						System.out.println("Mode: NEUTRAL");
					if(currentCommand==EmoState.IEE_MentalCommandAction_t.MC_PUSH.ToInt())
						System.out.println("Mode: PUSH");
					if(currentCommand==EmoState.IEE_MentalCommandAction_t.MC_PULL.ToInt())
						System.out.println("Mode: PULL");
					if(currentCommand==EmoState.IEE_MentalCommandAction_t.MC_LEFT.ToInt()){
						System.out.println("Mode: LEFT " + EmoState.INSTANCE.IS_MentalCommandGetCurrentActionPower(eState));
					}
					if(currentCommand==EmoState.IEE_MentalCommandAction_t.MC_RIGHT.ToInt()){
						System.out.println("Mode: RIGHT "  + EmoState.INSTANCE.IS_MentalCommandGetCurrentActionPower(eState));
					}
				}
	...
	public static void help() {
		...
		System.out.println("Press l to start live loop.");
	...
	public static boolean handleUserInput(IntByReference userId, IntByReference userCloudId, String profileName,
		...
		case "1": {
			...
			long listAction = action1 | action2; //| action3;
			...
			if (errorCode == EdkErrorCode.EDK_OK.ToInt())
				System.out.println(
						"Setting MentalCommand active actions (MC_LEFT | MC_RIGHT | MC_PUSH) for user " + userId.getValue());
			else
		...
		case "4.5": {
			System.out.println("Starting training PUSH!");
			Edk.INSTANCE.IEE_MentalCommandSetTrainingAction(userId.getValue(),
